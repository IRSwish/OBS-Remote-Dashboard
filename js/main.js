...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.addEventListener("obsMessage", e => {
        ...
        if(msg.op === 7 && msg.d.requestId === "stats"){
            ...
            const cpuFpsEl = document.getElementById("cpuFps");
            const liveStatusEl = document.getElementById("liveStatus");

            if(cpuFpsEl) cpuFpsEl.textContent = `CPU ${cpu}% • FPS ${fps} • RAM ${ram} MB • Dropped ${dropped} (${droppedPct}%)`;
            ...
            if(stats.streaming){
                ...
                if(liveStatusEl) liveStatusEl.textContent = `LIVE: ${h}:${m}:${s}`;
            } else {
                streamStartTime = null;
                if(liveStatusEl) liveStatusEl.textContent = "CONNECTED";
            }
        }
    });
});
